...
description: >
  ...
  native Windows executables. Covers the two recurring traps: (1) inline `-c`/`-e` script
  flags tripping the approval gate, and (2) MSYS `/c/...` paths getting mangled to
  `C:\c\...` when handed to Windows-native binaries. Prefer this over guessing shell syntax.
version: 1.1.0
metadata:
  hermes:
...
- **`search_files` (ripgrep) FAILS on absolute `C:\...` / `/c/...` paths** on this host —
  returns `IO error ... Системе не удается найти указанный путь (os error 3)`. For
  file-content search across a project, use `terminal` `grep -rn` / `grep -rl` instead.
  (Relative paths from the cwd sometimes work but are unreliable — don't depend on them.) **This bit repeatedly even after being documented — when a `search_files`/`vision_analyze` call on a `C:\...` path returns "cannot access" / "no such file", SWITCH to `terminal` with `/c/...` immediately, do NOT retry the same failing tool.**
- **Conduct: never "wait" without issuing a real call, and never poll a hung command in a loop.** Two corrections came from one session: (a) writing "жду результатов" / "продолжаю ждать" without a tool call produces no result — if you need output, MAKE the call (terminal/read_file). (b) Polling one slow/hung command (`dart analyze lib/` hung 100+ min on auto_route 11 codegen) dozens of times is a doom loop the user had to stop. **If a command runs >2× its normal time with no output, kill it (`process action=kill`) and verify another way** (per-folder analyze, `flutter build web`, or an ad-hoc temp verify script). One `wait` with a timeout is fine; 100 identical polls are not.
- **Complex compound one-liners get hardline-BLOCKED.** `grep -rl ... | wc -l` or
  long `&&`-chains tripped `BLOCKED (hardline)` here. Split into simple separate terminal
  calls, or use single-purpose `awk`/`sed`. Don't chain many pipes+filters in one command.
- **Flutter: Android SDK absent on this host** (`flutter build apk` → `[!] No Android SDK found`).
  That is an ENV gap, not a code bug. To still PROVE a Flutter tree compiles to a binary, enable
  web and build it: `flutter create . --platforms web` then `flutter build web --release`.
  Combined with `flutter analyze` (0 errors) + `flutter test` (green), a successful web build is
  sufficient build-proof when APK is impossible. `rsync` is also absent — use `cp -r` + `find -delete`.
