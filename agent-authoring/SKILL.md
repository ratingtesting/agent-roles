...
- **ЛИМИТ 60 СИМВОЛОВ** (проверено на практике: `skill_manage` отклоняет создание при большей длине —
  «must fit the 60-char system-prompt budget»). Одно предложение, триггер первым, точка в конце.
  Все детали «когда звать» переносить в тело, в секцию `## When to Use`.
- Пример годного: `Use when N docs must fuse into one spec.` (39 симв.)
