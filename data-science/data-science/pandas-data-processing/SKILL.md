...
For multi-format ingestion (CSV/Excel/Parquet/API dumps) behind a single dispatch, see
`references/abc-reader-hierarchy.md`. It covers two shapes — the ABC base class + registry
factory, and the lighter **plain-function decorator registry** (prefer it when readers are
stateless) — plus generalizing ONE `Registry` across several extension axes
(source / aggregation / output format), the `names()`-sorted vs `items()`-insertion-order
ordering bug, boundary validation carrying the row number, Decimal-until-render money handling,
and the edge cases to actually run (header-only file → no ZeroDivisionError in averages).

**When pandas is not the right tool here.** For a small tabular file with a per-row business
formula and currency arithmetic, stdlib `csv` + `Decimal` beats a pandas dependency: exact money
semantics, zero install, and the reuse ladder stops at L2. Reach for pandas when you need real
frame operations (joins, pivots, vectorized transforms, large files) — not merely to read a CSV
and sum a column. `references/abc-reader-hierarchy.md` shows the stdlib variant end to end.
